# Makefile for summit-cap
# Generated by AI QuickStart CLI

# Auto-detect compose tool: docker compose (v2) > podman-compose
...
build-images:
	...
	@podman build -f packages/api/Containerfile -t summit-cap-api:$(IMAGE_TAG) .
	@echo "Building UI image..."
	@podman build -f packages/ui/Containerfile -t summit-cap-ui:$(IMAGE_TAG) .
	@echo "✅ Images built successfully"

push-images:
	@NEED_BUILD=false && \
	if ! podman image exists summit-cap-api:$(IMAGE_TAG); then echo "API image not found"; NEED_BUILD=true; fi && \
	if ! podman image exists summit-cap-ui:$(IMAGE_TAG); then echo "UI image not found"; NEED_BUILD=true; fi && \
	...
	podman tag summit-cap-api:$(IMAGE_TAG) $$REGISTRY_URL/$(NAMESPACE)/summit-cap-api:$(IMAGE_TAG) && \
	if ! podman push $$REGISTRY_URL/$(NAMESPACE)/summit-cap-api:$(IMAGE_TAG); then echo ""; echo "❌ Failed to push API image. Common solutions:"; echo "  1. Ensure you have push permissions: oc policy add-role-to-user system:image-builder $$(oc whoami) -n $(NAMESPACE)"; echo "  2. Check if namespace exists: oc get project $(NAMESPACE)"; echo "  3. Try creating the project: oc new-project $(NAMESPACE)"; exit 1; fi && \
	echo "Tagging and pushing UI image..." && \
	podman tag summit-cap-ui:$(IMAGE_TAG) $$REGISTRY_URL/$(NAMESPACE)/summit-cap-ui:$(IMAGE_TAG) && \
	if ! podman push $$REGISTRY_URL/$(NAMESPACE)/summit-cap-ui:$(IMAGE_TAG); then echo ""; echo "❌ Failed to push UI image. Common solutions:"; echo "  1. Ensure you have push permissions: oc policy add-role-to-user system:image-builder $$(oc whoami) -n $(NAMESPACE)"; echo "  2. Check if namespace exists: oc get project $(NAMESPACE)"; echo "  3. Try creating the project: oc new-project $(NAMESPACE)"; exit 1; fi && \
